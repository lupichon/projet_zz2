...
{
    //DEBUG("%x is the state we're going in, %x is the state we're in\n", state,at86rf2xx_get_status(dev));
    ...
    if (state != AT86RF2XX_STATE_RX_AACK_ON) {
        ...
        if(waye_global_collection_phase) {
            ...
            //DEBUG("NOT RX_AACK_ON, PIN SLEEP = %d \n",gpio_read(dev->params.sleep_pin));
...
#ifdef MODULE_GNRC_WAYE
        if(waye_global_collection_phase && status == 0x11) {
            gpio_toggle(dev->params.sleep_pin);
        }
