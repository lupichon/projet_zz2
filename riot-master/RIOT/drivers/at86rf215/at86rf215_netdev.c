...
static uint32_t pre_buffer_loaded = 0;
static uint32_t post_buffer_loaded = 0;
static uint32_t fin_tx = 0;
...
static int _send(netdev_t *netdev, const iolist_t *iolist)
{
    ...
    at86rf215_t *dev = container_of(netdev_ieee802154, at86rf215_t, netdev);
    size_t len = 0;
    pre_buffer_loaded = rtt_get_counter();
    ...
    post_buffer_loaded = rtt_get_counter();
...
static void _isr(netdev_t *netdev)
{
    ...
    //DEBUG("BB IRQ = %x, STATE = %u\n", bb_irq_mask,dev->state);
    if(bb_irq_mask == 0x43 && dev->state == 3) {
        dev->state = 1;
        dev->flags &= ~AT86RF215_OPT_TX_PENDING;
    }
    int iter = 0;
    while (timeout || (bb_irq_mask & (BB_IRQ_RXFE | BB_IRQ_TXFE | BB_IRQ_RXAM))) {
    ...
    case AT86RF215_STATE_TX:
        //DEBUG("ON PASSE DANS 3?\n");
        //DEBUG("ON PASSE DANS 3? (%x) |%x| %x\n",bb_irq_mask,BB_IRQ_TXFE,bb_irq_mask & BB_IRQ_TXFE);
        if (!(bb_irq_mask & BB_IRQ_TXFE)) {
            DEBUG("TX: only TXFE expected (%x), %x\n", bb_irq_mask,bb_irq_mask & BB_IRQ_TXFE);
            break;
        }
        ...
        if (dev->flags & AT86RF215_OPT_ACK_REQUESTED) {
            dev->state = AT86RF215_STATE_TX_WAIT_ACK;
            _start_ack_timer(dev);
            //DEBUG("start ack timer\n");
        } else {
            fin_tx = rtt_get_counter();
            _tx_end(dev, NETDEV_EVENT_TX_COMPLETE);
            //DEBUG("[driver] Temps buffer : %lu, Temps total : %lu\n",TICKS_TO_US(post_buffer_loaded-pre_buffer_loaded),TICKS_TO_US(fin_tx-pre_buffer_loaded));
        }

        break;

    case AT86RF215_STATE_TX_WAIT_ACK:
        //printf("ON PASSE DANS 4?\n");
...
timeout:
        ...
        */
        if (dev->flags & AT86RF215_OPT_AGCH) {
            //printf("[at86rf215] Ack timeout postponed\n");
