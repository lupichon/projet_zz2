...
int brouilleur_send(gnrc_netif_t *netif);
static void waye_collect_noise_cb(void *arg);
static void _waye_msg_handler(gnrc_netif_t *netif, msg_t *msg);

static uint8_t header[MAC_HEADER_DRP_SIZE];
static uint8_t noise[1];
static bool do_not_scramble = true;
static uint32_t rx_end = 0;
static uint32_t tx_end = 0;
static uint32_t tx_start = 0;
static uint8_t nb_send = 0;
//static uint16_t nb_rx = 0;
...
int _send(gnrc_netif_t *netif, gnrc_pktsnip_t *pkt) {
    ...
    if(!do_not_scramble){
	    ...
	    if ((res = ieee802154_set_frame_hdr(mhr, src, src_len,
										dst, dst_len, dev_pan,
										dev_pan, flags, header[2])) == 0) {
		    DEBUG("_send_ieee802154: Error preperaring frame : %u\n",res);
		    return -EINVAL;
        }
...
int brouilleur_send(gnrc_netif_t *netif) {
	...
	reply_payload = gnrc_pktbuf_add(NULL,NULL, 0, 1);
	...
	netif_pkt = gnrc_netif_hdr_build(NULL, 0, NULL, 0);
    ...
    if (event == NETDEV_EVENT_ISR) {
        ...
        switch (event) {
            ...
            case NETDEV_EVENT_RX_COMPLETE:
                ...
                //printf("prout %u, ptr = %u\n",nb_rx,*ptr);
                if(*ptr == GNRC_WAYE_FRAMETYPE_DATA_POLL && nb_send < 5) {
                    nb_send++;
                        uint32_t send_noise = rx_end + RTT_US_TO_TICKS(4200);
                        rtt_set_alarm(send_noise,waye_collect_noise_cb,netif);
                        //brouilleur_send(netif);
                }
                gnrc_pktbuf_release(pkt);
            ...
            case NETDEV_EVENT_TX_STARTED:
                tx_start = rtt_get_counter(); 
                gnrc_netif_set_tx_feedback(netif, TX_FEEDBACK_UNDEF);
				gnrc_netif_set_rx_started(netif, false); 
                break;

            case NETDEV_EVENT_TX_COMPLETE:
                tx_end = rtt_get_counter();
                printf("Frame scrambled ! %lu \n",TICKS_TO_US(tx_end-tx_start));
...
static void _waye_msg_handler(gnrc_netif_t *netif, msg_t *msg) {
	//printf("i am inside message handler\n");	
	switch (msg->type) {
...
static int _waye_brouilleur_init(gnrc_netif_t *netif) {
    ...
    uint8_t txafcs = at86rf215_reg_read(at86rf215_dev, at86rf215_dev->BBC->RG_PC);
    txafcs &= ~PC_TXAFCS_MASK;
    at86rf215_reg_write(at86rf215_dev,at86rf215_dev->BBC->RG_PC,txafcs);
    at86rf215_dev->retries_max = 0;
    at86rf215_dev->retries = 0;
    memset(&noise,255,sizeof(noise));
    rtt_init();
    rtt_set_counter(0);
    return 0;   
}
