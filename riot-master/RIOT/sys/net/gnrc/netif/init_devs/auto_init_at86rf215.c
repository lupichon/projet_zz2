...
#elif defined(TEST_RECV)
    puts("MODULE_GNRC_WAYE");
        gnrc_netif_waye_create(netif, stack,
                                AT86RF215_MAC_STACKSIZE,
                                prio, name, netdev);
...
void auto_init_at86rf215(void)
{
    ...
    for (unsigned j = 0; j < AT86RF215_NUM; ++j) {
        ...
        _setup_netif(netif_09, dev_09, stack_09, AT86RF215_MAC_PRIO_SUBGHZ, "at86rf215-waye[sub GHz]");

        /* setup 2.4-GHz interface */
        _setup_netif(netif_24, dev_24, stack_24, AT86RF215_MAC_PRIO, "at86rf215-waye[2.4 GHz]");
    }
}
/** @} */
