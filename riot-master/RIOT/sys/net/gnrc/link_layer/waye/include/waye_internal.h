 ...
 * @param[in] size			number of addresses in the array
 * @param[in] sink_addr     ptr to the sink address
 * 
 * @return	0 on success
 * @return	-ENOBUFS, if no space in packet buffer
 * @return	Any negative error code reported by _gnrc_waye_transmit
...
int gnrc_waye_send_mixed_poll(gnrc_netif_t *netif, gnrc_waye_frame_data_mixed_poll_t *data_poll, le_uint16_t addr_list[], gnrc_waye_frame_actuator_order_t * actuator_order, uint8_t order_count,uint8_t *sink_addr);
